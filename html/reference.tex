...
~\\'wait\_threads' can be \{ 0=never ~$|$~ 1=on parser return ~$|$~ 1=immediately \}.
~\\If 'wait\_threads' is set to '2', the interpreter blocks until all threads have returned.
~\\Otherwise, it continues the execution flow while threads are running on the background.
~\\~\\\textbf{Default values}: {\small \texttt{'wait\_threads=2'.}}
